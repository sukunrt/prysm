...
func (b *BeaconStateGloas) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	if size := len(b.ProposerLookahead); size != 16 {
		err = ssz.ErrVectorLengthFn("--.ProposerLookahead", size, 16)
		return
	}
	for ii := 0; ii < 16; ii++ {
	...
	if size := len(b.BuilderPendingPayments); size != 16 {
		err = ssz.ErrVectorLengthFn("--.BuilderPendingPayments", size, 16)
		return
	}
	for ii := 0; ii < 16; ii++ {
	...
	// Field (45) 'PtcWindow'
	if size := len(b.PtcWindow); size != 24 {
		err = ssz.ErrVectorLengthFn("--.PtcWindow", size, 24)
		return
	}
	for ii := 0; ii < 24; ii++ {
...
func (b *BeaconStateGloas) UnmarshalSSZ(buf []byte) error {
	...
	// Field (37) 'ProposerLookahead'
	b.ProposerLookahead = ssz.ExtendUint(b.ProposerLookahead, 16)
	for ii := 0; ii < 16; ii++ {
		b.ProposerLookahead[ii] = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.ValidatorIndex](buf[2736741:2736869][ii*8 : (ii+1)*8])
	}

	// Offset (38) 'Builders'
	if o38 = ssz.ReadOffset(buf[2736869:2736873]); o38 > size || o36 > o38 {
		return ssz.ErrOffset
	}

	// Field (39) 'NextWithdrawalBuilderIndex'
	b.NextWithdrawalBuilderIndex = ssz.UnmarshallUint[github_com_OffchainLabs_prysm_v7_consensus_types_primitives.BuilderIndex](buf[2736873:2736881])

	// Field (40) 'ExecutionPayloadAvailability'
	if cap(b.ExecutionPayloadAvailability) == 0 {
		b.ExecutionPayloadAvailability = make([]byte, 0, len(buf[2736881:2737905]))
	}
	b.ExecutionPayloadAvailability = append(b.ExecutionPayloadAvailability, buf[2736881:2737905]...)

	// Field (41) 'BuilderPendingPayments'
	b.BuilderPendingPayments = make([]*BuilderPendingPayment, 16)
	for ii := 0; ii < 16; ii++ {
		if b.BuilderPendingPayments[ii] == nil {
			b.BuilderPendingPayments[ii] = new(BuilderPendingPayment)
		}
		if err = b.BuilderPendingPayments[ii].UnmarshalSSZ(buf[2737905:2738737][ii*52 : (ii+1)*52]); err != nil {
			return err
		}
	}

	// Offset (42) 'BuilderPendingWithdrawals'
	if o42 = ssz.ReadOffset(buf[2738737:2738741]); o42 > size || o38 > o42 {
		return ssz.ErrOffset
	}

	// Offset (43) 'LatestExecutionPayloadBid'
	if o43 = ssz.ReadOffset(buf[2738741:2738745]); o43 > size || o42 > o43 {
		return ssz.ErrOffset
	}

	// Offset (44) 'PayloadExpectedWithdrawals'
	if o44 = ssz.ReadOffset(buf[2738745:2738749]); o44 > size || o43 > o44 {
		return ssz.ErrOffset
	}

	// Field (45) 'PtcWindow'
	b.PtcWindow = make([]*PTCs, 24)
	for ii := 0; ii < 24; ii++ {
		if b.PtcWindow[ii] == nil {
			b.PtcWindow[ii] = new(PTCs)
		}
		if err = b.PtcWindow[ii].UnmarshalSSZ(buf[2738749:2837053][ii*4096 : (ii+1)*4096]); err != nil {
			return err
		}
	}
