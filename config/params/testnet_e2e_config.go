...
// E2EDecoupledTestConfig is E2EMainnetTestConfig on the decoupled preset
// (mainnet SSZ sizes, 8-slot epochs). It only overrides values that are not
// preset-checked: VerifyPreset at node startup rejects any drift between this
// config and the compiled-in decoupled field parameters.
func E2EDecoupledTestConfig() *BeaconChainConfig {
	e2eConfig := DecoupledConfig()
	e2eConfig.DepositContractAddress = "0x4242424242424242424242424242424242424242"
	e2eConfig.Eth1FollowDistance = 8
	...
	e2eConfig.ShardCommitteePeriod = 4
	e2eConfig.MinValidatorWithdrawabilityDelay = 1
	// Not preset-checked; keeps the withdrawal evaluator inside a 10-epoch run
	// (exit at 4 -> withdrawal verified at 8), as in the minimal e2e config.
	...
	// Prysm constants.
	e2eConfig.ConfigName = EndToEndDecoupledName
	e2eConfig.GenesisForkVersion = []byte{0, 0, 0, 252}
	e2eConfig.AltairForkVersion = []byte{1, 0, 0, 252}
	e2eConfig.BellatrixForkVersion = []byte{2, 0, 0, 252}
	e2eConfig.CapellaForkVersion = []byte{3, 0, 0, 252}
	e2eConfig.DenebForkVersion = []byte{4, 0, 0, 252}
	e2eConfig.ElectraForkVersion = []byte{5, 0, 0, 252}
	e2eConfig.FuluForkVersion = []byte{6, 0, 0, 252}
	e2eConfig.GloasForkVersion = []byte{7, 0, 0, 252}
	e2eConfig.HezeForkVersion = []byte{8, 0, 0, 252}
